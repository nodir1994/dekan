...
import { updateStudent, fetchStudentsById } from "@/utils/api/students/create";
...
const formState: any = reactive({
  ...
  address_father: null,
  phone_mother: null,
  workplace_mother: null,
  workplace_father: null,
  _method: "PUT",
});
...
async function fetchData() {
  const res = await fetchStudentsById(paramsID);
  // console.log(res)
  Object.assign(formState, res);
  formState.academic_year_id = res?.semester?.academic_year_id;
  formState.country_id = res?.country?.id;
}
...
async function onSubmit() {
  try {
    ...
    Object.getOwnPropertyNames(formState).forEach((prop) => {
      if (!["id", "citizenship", "user_id"].includes(prop) && formState[prop]) {
        if (!(prop == "image")) {
          ...
          // console.log(formState[prop], 'formData')
        } else {
          if (isFile(formState[prop])) {
            formData.append(prop, formState[prop]);
          }
        }
      }
    ...
    const res = await updateStudent(paramsID, formData);
    notif.success(t("Student_name_updated_successfully"));
    router.back();
