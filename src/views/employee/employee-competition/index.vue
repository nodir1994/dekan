...
import { onBeforeMount, ref, reactive, watch, onMounted, computed } from "vue";
import { useI18n } from "vue-i18n";
import Swal from "sweetalert2";
import { Modal } from "bootstrap";
import { useNotyf } from "@/composable/useNotyf";
import {
  employeeCompetitionList,
  deleteEmployeeCompetition,
  changeStatus,
...
async function onRemove(id: number) {
  ...
  try {
    ...
    if (res.isConfirmed) {
      await deleteEmployeeCompetition(id);
      await fetchList();
...
async function fetchList() {
  const res = await employeeCompetitionList(apiParams);
  apiData.data = null;
...
<template>
  <div
    pagetitle="'Tanlovdan o'tish oshirish'"
    class="employee-competition-page"
  >
    <!-----------START PAGE LIST HEADER TOP ------------------------>
    <div class="card panel-header-bg">
      <div class="card-body">
        <div class="panel-header">
          <h5 class="card-title">{{ $t("Employee_Competition") }}</h5>
              ...
              >
                ...
                <td>
                  <!-- <div class="form-check form-switch my-auto">
                    <input class="form-check-input" type="checkbox" :checked="element.status" :disabled="isLoading" @change="updateStatus(element.id)" />
                  </div> -->
                  <div class="form-check form-switch my-auto">
                    <!-- {{ element.status }} -->
                    <input
    ...
    <EmployeeCompetitionFormModal
      :employee-competition-id="selectedId"
...
<style lang="scss">
.employee-competition-page {
  .table-row {
    grid-template-columns: 50px auto 13% 14% 14% 12% 14% 70px 70px;

    @include mobile() {
      grid-template-columns: 50px 200px 100px 100px 100px 90px 100px 70px 60px;
    }
  }
