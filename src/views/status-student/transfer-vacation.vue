...
import {
  ...
  getCommandList,
  createAcademicLeave,
} from "@/utils/api/student-action/academic-leave";
...
const apiData: any = reactive({
  ...
  reference_id: null, //jarayon sababi
  command_id: null, //buyruq
  current_page: 1,
  data: [],
  links: [],
  page: 1,
...
onMounted(async () => {
  ...
  sideFormOptions.referenceOptions = await getReasonList().then(
    (res) => res.result
  );
  sideFormOptions.orderOptions = await getCommandList().then((res) =>
...
watch(
  ...
  async () => {
    ...
    if (apiData.edu_plan_id) {
      ...
      sideFormOptions.academicYearsOptions = res?.result?.data.map((item) => {
        return { id: item.id, name: item.name };
      });
      let res2 = await academicGroupById(apiData.edu_plan_id);
      sideFormOptions.academicGroupOptions = res2?.result?.data.map((item) => {
        return { id: item.id, name: item.name };
      });
    }
  }
...
watch(
  ...
  async () => {
    ...
    if (apiData.edu_plan_id && apiData.academic_year_id) {
      ...
      sideFormOptions.semesterOptions = res?.result?.data.map((item) => {
        return {
          id: item.id,
          name:
            item.course +
...
async function attachData() {
  try {
    ...
    Swal.fire({
      ...
      if (result.value) {
        await createAcademicLeave(apiData);
...
<template>
  <div pagetitle="'Akademik tatil'" class="transfer-vacation-page">
    <div class="card panel-header-bg">
      <div class="card-body">
        <div class="panel-header">
          <h5 class="card-title">{{ $t("academic_leave") }}</h5>
            ...
            <a-form>
              <div class="mb-3" name="nameFaculty">
                ...
                <a-select :filter-option="helpers.filterOptionByName" show-search allow-clear
                  ...
                  :options="sideFormOptions.referenceOptions" :placeholder="t('choose__reason')">
                </a-select>
                <span v-if="errors.reference_id" class="text-danger small">
              ...
              <div class="mb-3" name="nameFaculty">
                ...
                <a-input v-model:value="apiData.dateCommand" placeholder="YYYY-MM-DD" disabled
