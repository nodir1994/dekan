...
import { getListStudentCreate } from "@/utils/api/archive/transcript";
...
async function fetchList(page?: number) {
  ...
  if (loader.value) {
    loader.value = false;
    const res = await getListStudentCreate(datas);
        ...
        <div class="panel-header">
          ...
          <div class="panel-toggles">
            <div class="filterBlock">
              <button v-b-toggle.collapse-3 class="btns c-filter py-1.5 px-4 me-2 d-inline-block">
                <i class="mdi mdi-filter font-size-14 align-middle me-1"></i>{{ $t("Filter") }}
              </button>
              <router-link to="/archive/transcript" class="btns black-c py-1.5 px-4 d-inline-block">
          ...
          <div class="card-body">
            <div v-if="apiData.data.length" class="table-responsive">
              <table class="content-table">
                <thead>
                  <tr class="table-row-transcript-index23">
                    <th>#</th>
                    <th>{{ $t("Student") }}</th>
                ...
                <tbody>
                  <tr class="table-row-transcript-index23" v-for="(element, index) in apiData.data" :key="element?.id">
                    <td>
                      {{
                    ...
                    <td>
                      <div>
                        {{ element?.number }} <br />
                        <span style="color: #ccc">{{ element?.given_date }}</span>
                      </div>
                    </td>
                    <td>
                      <div class="action__td">
                        <router-link class="btns c-save py-1 px-2 me-2" :to="'/archive/transcript/edit?student_id=' + element?.id
                          ">
...
<style lang="scss">
.table-row-transcript-create {
  display: grid;
  width: 100% !important;
  grid-gap: 0;
  grid-template-columns: 50px auto 200px 140px 200px 150px;
}
...
.table-row-transcript-index23 {
  display: grid;
  width: 100% !important;
  grid-gap: 0;
  grid-template-columns: 50px auto 200px 240px 250px 120px 120px;
}
</style>
