...
import { createMessage, saveDraftMessage } from "@/utils/api/messages";
...
const selectedUsers = computed(() => {
  ...
  if (selectedEmployees.value.length > 0) {
    res += selectedEmployees.value.map((item) => item.full_name).join(", ");
  }
  ...
  if (selectedStudents.value.length > 0) {
    ...
    res += selectedStudents.value.map((item) => item.full_name).join(", ");
  }
...
async function onSubmit() {
  try {
    ...
    if (formState.academic_group_id)
      formData.append("academic_group_id", formState.academic_group_id);
    const users = [
...
async function saveMessageAsDraft() {
  try {
    ...
    await saveDraftMessage(formData);
          ...
          <div class="card-body">
            <form @submit.prevent="onSubmit">
              <div class="row">
                ...
                <template v-if="!(route.query.id && route.query.name)">
                  <div class="col-md-2">
                    <button
            ...
            <EmployeesListModal
              ...
              @update:department-id="formState.department_id = $event"
            />
            <!-------END MODAL--xodim-------------------------------------->
            ...
            <StudentsListModal
              ...
              @update:academic-group-id="formState.academic_group_id = $event"
            />
            <!-------END MODAL---------------------------------------->
          </div>
        </div>
